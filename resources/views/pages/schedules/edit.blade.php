...
@section('content')
    <x-common.page-breadcrumb pageTitle="Editar Horario" :breadcrumbs="[['name' => 'Horarios', 'url' => route('schedules.index')], ['name' => 'Editar', 'url' => null]]" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Modifica los detalles del horario asignado
            </p>
        </div>
        ...
        <form action="{{ route('schedules.update', $horario) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <select id="employee_id" name="employee_id" required
                        ...
                        @foreach ($empleados as $empleado)
                            <option value="{{ $empleado->id }}"
                                {{ old('employee_id', $horario->employee_id) == $empleado->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" id="scheduled_date" name="scheduled_date"
                        value="{{ old('scheduled_date', $horario->scheduled_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="time" id="start_time" name="start_time"
                        value="{{ old('start_time', $horario->start_time->format('H:i')) }}" required
                ...
                <div>
                    ...
                    <input type="time" id="end_time" name="end_time"
                        value="{{ old('end_time', $horario->end_time->format('H:i')) }}" required
                    ...
                    <select id="schedule_template_id" name="schedule_template_id"
                        ...
                        @foreach ($plantillas as $plantilla)
                            <option value="{{ $plantilla->id }}"
                                {{ old('schedule_template_id', $horario->schedule_template_id) == $plantilla->id ? 'selected' : '' }}>
                    ...
                    <div class="flex items-center gap-4">
                        <label class="flex items-center">
                            <input type="radio" name="is_published" value="0"
                                {{ old('is_published', $horario->is_published ? '1' : '0') == '0' ? 'checked' : '' }}
                        ...
                        <label class="flex items-center">
                            <input type="radio" name="is_published" value="1"
                                {{ old('is_published', $horario->is_published ? '1' : '0') == '1' ? 'checked' : '' }}
                                class="h-4 w-4 text-brand-600 focus:ring-brand-500 border-gray-300">
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="3" placeholder="Notas adicionales sobre este horario..."
                    class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{{ old('notes', $horario->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">Información del Registro</h4>
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 text-sm">
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Creado por:</span>
                        <span class="ml-2 text-gray-900 dark:text-white">{{ $horario->creator->name ?? 'Sistema' }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Fecha de creación:</span>
                        <span
                            class="ml-2 text-gray-900 dark:text-white">{{ $horario->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    @if ($horario->updated_at != $horario->created_at)
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">Última modificación:</span>
                            <span
                                class="ml-2 text-gray-900 dark:text-white">{{ $horario->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                    @endif
                </div>
            </div>
